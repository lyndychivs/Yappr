...
public sealed class TldrPromptBuilderTests
{
    ...
    public void Build_WhenTranscriptExceedsBudget_DropsOldestMessagesFirst()
    {
        ...
        {
            messages.Add(new ChannelMessage("User", $"message number {i} with some padding text to grow the transcript", DateTimeOffset.UtcNow.AddSeconds(i)));
        }
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(prompt.Length, Is.LessThan(TldrPromptBuilder.MaxPromptCharacters + 2_000));
        });
    }
}
